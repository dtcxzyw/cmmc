...
pcrel2037:
	...
	sd s8, 32(sp)
	addi s8, s7, %pcrel_lo(pcrel2037)
	sd s1, 40(sp)
	addi s1, a0, %pcrel_lo(pcrel2036)
	sd s6, 48(sp)
pcrel2038:
	auipc a0, %pcrel_hi(cmmc_parallel_body_2)
	sd s9, 56(sp)
	addi s6, a0, %pcrel_lo(pcrel2038)
	mv s9, zero
	li a0, 125
	sd s2, 64(sp)
	slli s0, a0, 5
	addi s2, a1, %pcrel_lo(pcrel2035)
	sd s3, 72(sp)
	slli s3, s0, 1
	sd s4, 80(sp)
	sh1add s4, s0, s0
...
pcrel2039:
	...
	addi a2, a3, %pcrel_lo(pcrel2039)
	jal cmmcParallelFor
	li a1, 1000
	mv a0, zero
pcrel2040:
	auipc a3, %pcrel_hi(cmmc_parallel_body_1)
	addi a2, a3, %pcrel_lo(pcrel2040)
	jal cmmcParallelFor
	mv s9, zero
	mv s5, s1
	mv a0, s1
