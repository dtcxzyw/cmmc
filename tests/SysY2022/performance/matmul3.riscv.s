...
pcrel2041:
	...
	sd s8, 32(sp)
	addi s8, s7, %pcrel_lo(pcrel2041)
	sd s1, 40(sp)
	addi s1, a0, %pcrel_lo(pcrel2040)
	sd s6, 48(sp)
pcrel2042:
	auipc a0, %pcrel_hi(cmmc_parallel_body_3)
	addi s6, a1, %pcrel_lo(pcrel2039)
	sd s9, 56(sp)
	mv s9, zero
	sd s2, 64(sp)
	addi s2, a0, %pcrel_lo(pcrel2042)
	sd s3, 72(sp)
	li a0, 125
	sd s4, 80(sp)
	slli s0, a0, 5
