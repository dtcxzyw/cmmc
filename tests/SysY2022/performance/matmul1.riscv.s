...
pcrel1985:
	...
	sd s8, 32(sp)
	addi s8, s7, %pcrel_lo(pcrel1985)
	sd s1, 40(sp)
	addi s1, a0, %pcrel_lo(pcrel1984)
	sd s6, 48(sp)
pcrel1986:
	auipc a0, %pcrel_hi(cmmc_parallel_body_2)
	sd s9, 56(sp)
	addi s6, a0, %pcrel_lo(pcrel1986)
	mv s9, zero
	li a0, 125
	sd s2, 64(sp)
	slli s0, a0, 5
	addi s2, a1, %pcrel_lo(pcrel1983)
	sd s3, 72(sp)
	slli s3, s0, 1
	sd s4, 80(sp)
	sh1add s4, s0, s0
