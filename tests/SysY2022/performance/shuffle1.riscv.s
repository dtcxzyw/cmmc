...
pcrel319:
	...
	mv s0, a0
	addi a0, a1, %pcrel_lo(pcrel319)
	mv s3, a0
	jal getarray
pcrel320:
	auipc a1, %pcrel_hi(values)
	mv s4, a0
	addi a0, a1, %pcrel_lo(pcrel320)
	mv s5, a0
	jal getarray
pcrel321:
	auipc a1, %pcrel_hi(requests)
	addi a0, a1, %pcrel_lo(pcrel321)
	mv s2, a0
	jal getarray
	mv s1, a0
	li a0, 78
...
pcrel324:
	...
	addi a2, a0, %pcrel_lo(pcrel323)
	addi a3, a1, %pcrel_lo(pcrel324)
pcrel325:
	auipc a1, %pcrel_hi(value)
	addi a0, a1, %pcrel_lo(pcrel325)
pcrel326:
	auipc a1, %pcrel_hi(next)
	addi a4, a1, %pcrel_lo(pcrel326)
	addi a1, a5, %pcrel_lo(pcrel322)
	ble s4, zero, label14
	mv t0, zero
	mv a5, zero
	mv t1, s3
