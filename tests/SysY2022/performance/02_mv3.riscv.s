.attribute arch, "rv64i2p1_m2p0_a2p1_f2p2_d2p2_c2p0_zicsr2p0_zifencei2p0_zba1p0_zbb1p0"
.data
.bss
.align 8
A:
	.zero	16160400
.align 8
B:
	.zero	8040
.align 8
C:
	.zero	8040
.align 8
cmmc_parallel_body_payload_0:
	.zero	4
.align 8
cmmc_parallel_body_payload_1:
	.zero	4
.align 8
cmmc_parallel_body_payload_2:
	.zero	4
.align 8
cmmc_parallel_body_payload_3:
	.zero	4
.text
.p2align 2
.globl main
main:
	# stack usage: CalleeArg[0] Local[0] RegSpill[24] CalleeSaved[104]
	addi sp, sp, -128
	sd ra, 0(sp)
	sd s11, 8(sp)
	sd s0, 16(sp)
	sd s5, 24(sp)
	sd s1, 32(sp)
	sd s6, 40(sp)
	sd s8, 48(sp)
	sd s2, 56(sp)
	sd s3, 64(sp)
	sd s4, 72(sp)
	sd s7, 80(sp)
	sd s9, 88(sp)
	sd s10, 96(sp)
	jal getint
pcrel516:
	auipc s6, %pcrel_hi(cmmc_parallel_body_payload_3)
pcrel517:
	auipc s5, %pcrel_hi(cmmc_parallel_body_payload_2)
pcrel518:
	auipc s3, %pcrel_hi(cmmc_parallel_body_payload_0)
pcrel519:
	auipc a3, %pcrel_hi(cmmc_parallel_body_0)
pcrel520:
	auipc a1, %pcrel_hi(cmmc_parallel_body_1)
pcrel521:
	auipc s4, %pcrel_hi(cmmc_parallel_body_payload_1)
	li s2, 50
	mv s11, a0
	addi a2, a3, %pcrel_lo(pcrel519)
	sd a0, 104(sp)
pcrel522:
	auipc a0, %pcrel_hi(cmmc_parallel_body_2)
	sd a2, 112(sp)
	addi s0, a0, %pcrel_lo(pcrel522)
	addi a2, a1, %pcrel_lo(pcrel520)
	li a0, 1005
pcrel523:
	auipc a1, %pcrel_hi(cmmc_parallel_body_3)
	slli s8, a0, 3
	sd a2, 120(sp)
	addi s1, a1, %pcrel_lo(pcrel523)
	ble s11, zero, label381
pcrel524:
	auipc a0, %pcrel_hi(A)
	mv s9, zero
	addi s7, a0, %pcrel_lo(pcrel524)
	mv s10, s7
	mv s11, zero
	j label397
.p2align 2
label451:
	addiw s9, s9, 1
	ld s11, 104(sp)
	ble s11, s9, label381
	add s7, s7, s8
	mv s11, zero
	mv s10, s7
.p2align 2
label397:
	jal getint
	addiw a1, s11, 1
	sw a0, 0(s10)
	ld s11, 104(sp)
	ble s11, a1, label451
	addi s10, s10, 4
	mv s11, a1
	j label397
label381:
	ld s11, 104(sp)
	ble s11, zero, label382
pcrel525:
	auipc a1, %pcrel_hi(B)
	mv s8, zero
	addi s7, a1, %pcrel_lo(pcrel525)
.p2align 2
label389:
	jal getint
	addiw a1, s8, 1
	sw a0, 0(s7)
	ld s11, 104(sp)
	ble s11, a1, label382
	addi s7, s7, 4
	mv s8, a1
	j label389
label382:
	li a0, 59
	jal _sysy_starttime
	mv s8, zero
pcrel526:
	auipc a1, %pcrel_hi(C)
	addi s7, a1, %pcrel_lo(pcrel526)
.p2align 2
label383:
	ld s11, 104(sp)
	ble s11, zero, label386
	mv a0, zero
pcrel527:
	auipc s3, %pcrel_hi(cmmc_parallel_body_payload_0)
	sw s11, %pcrel_lo(pcrel527)(s3)
	ld a2, 112(sp)
	mv a1, s11
	jal cmmcParallelFor
	mv a0, zero
pcrel528:
	auipc s4, %pcrel_hi(cmmc_parallel_body_payload_1)
	sw s11, %pcrel_lo(pcrel528)(s4)
	ld a2, 120(sp)
	mv a1, s11
	jal cmmcParallelFor
	mv a0, zero
pcrel529:
	auipc s5, %pcrel_hi(cmmc_parallel_body_payload_2)
	sw s11, %pcrel_lo(pcrel529)(s5)
	mv a1, s11
	mv a2, s0
	jal cmmcParallelFor
	mv a0, zero
pcrel530:
	auipc s6, %pcrel_hi(cmmc_parallel_body_payload_3)
	sw s11, %pcrel_lo(pcrel530)(s6)
	mv a1, s11
	mv a2, s1
	jal cmmcParallelFor
.p2align 2
label386:
	addiw s8, s8, 1
	blt s8, s2, label383
	li a0, 67
	jal _sysy_stoptime
	ld s11, 104(sp)
	mv a0, s11
	mv a1, s7
	jal putarray
	mv a0, zero
	ld ra, 0(sp)
	ld s11, 8(sp)
	ld s0, 16(sp)
	ld s5, 24(sp)
	ld s1, 32(sp)
	ld s6, 40(sp)
	ld s8, 48(sp)
	ld s2, 56(sp)
	ld s3, 64(sp)
	ld s4, 72(sp)
	ld s7, 80(sp)
	ld s9, 88(sp)
	ld s10, 96(sp)
	addi sp, sp, 128
	ret
.p2align 2
cmmc_parallel_body_0:
	mv a2, a0
	addiw a4, a0, 3
pcrel114:
	auipc a0, %pcrel_hi(C)
	addi a3, a0, %pcrel_lo(pcrel114)
	ble a1, a4, label16
	addiw t0, a2, 15
	addiw a4, a1, -3
	addiw a5, a1, -18
	bge t0, a4, label37
	sh2add a0, a2, a3
	j label12
.p2align 2
label15:
	addi a0, a0, 64
.p2align 2
label12:
	addiw a2, a2, 16
	sd zero, 0(a0)
	sd zero, 8(a0)
	sd zero, 16(a0)
	sd zero, 24(a0)
	sd zero, 32(a0)
	sd zero, 40(a0)
	sd zero, 48(a0)
	sd zero, 56(a0)
	bgt a5, a2, label15
	mv a0, a2
label3:
	ble a4, a0, label16
	sh2add a5, a0, a3
	mv a2, a0
	j label7
label10:
	addi a5, a5, 16
label7:
	addiw a2, a2, 4
	sw zero, 0(a5)
	sw zero, 4(a5)
	sw zero, 8(a5)
	sw zero, 12(a5)
	bgt a4, a2, label10
label16:
	ble a1, a2, label23
	sh2add a0, a2, a3
	j label19
label22:
	addi a0, a0, 4
label19:
	addiw a2, a2, 1
	sw zero, 0(a0)
	bgt a1, a2, label22
label23:
	ret
label37:
	mv a0, a2
	mv a2, zero
	j label3
.p2align 2
cmmc_parallel_body_1:
	mv a3, a0
	mv a5, a1
pcrel186:
	auipc a4, %pcrel_hi(A)
pcrel187:
	auipc t1, %pcrel_hi(cmmc_parallel_body_payload_1)
pcrel188:
	auipc t2, %pcrel_hi(B)
	li a0, 1005
	addi a2, a4, %pcrel_lo(pcrel186)
	slli t0, a0, 3
	lw a0, %pcrel_lo(pcrel187)(t1)
	mulw a1, a3, t0
pcrel189:
	auipc t1, %pcrel_hi(C)
	add a4, a2, a1
	addi a2, t1, %pcrel_lo(pcrel189)
	addi a1, t2, %pcrel_lo(pcrel188)
	mv t1, a4
	mv t2, zero
	lw t3, 0(a4)
	beq t3, zero, label122
	sh2add t6, a3, a2
	lw t5, 0(a1)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
.p2align 2
label122:
	addiw t2, t2, 1
	ble a0, t2, label149
.p2align 2
label123:
	addi t1, t1, 4
	lw t3, 0(t1)
	beq t3, zero, label122
	sh2add a6, t2, a1
	sh2add t6, a3, a2
	addiw t2, t2, 1
	lw t5, 0(a6)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
	bgt a0, t2, label123
	addiw a3, a3, 1
	bgt a5, a3, label125
	j label126
.p2align 2
label149:
	addiw a3, a3, 1
	ble a5, a3, label126
.p2align 2
label125:
	add a4, a4, t0
	mv t2, zero
	lw t3, 0(a4)
	mv t1, a4
	beq t3, zero, label122
	sh2add t6, a3, a2
	li t2, 1
	lw t5, 0(a1)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
	bgt a0, t2, label123
	j label149
label126:
	ret
.p2align 2
cmmc_parallel_body_2:
	mv a2, a0
	addiw a4, a0, 3
pcrel304:
	auipc a0, %pcrel_hi(B)
	addi a3, a0, %pcrel_lo(pcrel304)
	ble a1, a4, label205
	addiw t0, a2, 15
	addiw a4, a1, -3
	addiw a5, a1, -18
	bge t0, a4, label226
	sh2add a0, a2, a3
	j label193
.p2align 2
label196:
	addi a0, a0, 64
.p2align 2
label193:
	addiw a2, a2, 16
	sd zero, 0(a0)
	sd zero, 8(a0)
	sd zero, 16(a0)
	sd zero, 24(a0)
	sd zero, 32(a0)
	sd zero, 40(a0)
	sd zero, 48(a0)
	sd zero, 56(a0)
	bgt a5, a2, label196
	mv a0, a2
label197:
	ble a4, a0, label205
	sh2add a5, a0, a3
	mv a2, a0
label201:
	addiw a2, a2, 4
	sw zero, 0(a5)
	sw zero, 4(a5)
	sw zero, 8(a5)
	sw zero, 12(a5)
	ble a4, a2, label205
	addi a5, a5, 16
	j label201
label205:
	ble a1, a2, label212
	sh2add a0, a2, a3
	j label208
label211:
	addi a0, a0, 4
label208:
	addiw a2, a2, 1
	sw zero, 0(a0)
	bgt a1, a2, label211
label212:
	ret
label226:
	mv a0, a2
	mv a2, zero
	j label197
.p2align 2
cmmc_parallel_body_3:
	mv a3, a0
	mv a5, a1
pcrel376:
	auipc a4, %pcrel_hi(A)
pcrel377:
	auipc t1, %pcrel_hi(cmmc_parallel_body_payload_3)
pcrel378:
	auipc t2, %pcrel_hi(C)
	li a0, 1005
	addi a2, a4, %pcrel_lo(pcrel376)
	slli t0, a0, 3
	lw a0, %pcrel_lo(pcrel377)(t1)
	mulw a1, a3, t0
pcrel379:
	auipc t1, %pcrel_hi(B)
	add a4, a2, a1
	addi a2, t1, %pcrel_lo(pcrel379)
	addi a1, t2, %pcrel_lo(pcrel378)
	mv t1, a4
	mv t2, zero
	lw t3, 0(a4)
	beq t3, zero, label312
	sh2add t6, a3, a2
	lw t5, 0(a1)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
.p2align 2
label312:
	addiw t2, t2, 1
	ble a0, t2, label339
.p2align 2
label313:
	addi t1, t1, 4
	lw t3, 0(t1)
	beq t3, zero, label312
	sh2add a6, t2, a1
	sh2add t6, a3, a2
	addiw t2, t2, 1
	lw t5, 0(a6)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
	bgt a0, t2, label313
	addiw a3, a3, 1
	bgt a5, a3, label315
	j label316
.p2align 2
label339:
	addiw a3, a3, 1
	ble a5, a3, label316
.p2align 2
label315:
	add a4, a4, t0
	mv t2, zero
	lw t3, 0(a4)
	mv t1, a4
	beq t3, zero, label312
	sh2add t6, a3, a2
	li t2, 1
	lw t5, 0(a1)
	mulw t4, t3, t5
	amoadd.w.aqrl a6, t4, (t6)
	bgt a0, t2, label313
	j label339
label316:
	ret
