...
bool reorderBlock(Function& func) {
    ...
    for (auto& block: func.blocks()){
        ...
        std::priority_queue<Value*, std::vector<Value*>, std::function<bool(Value*, Value*)>> 
            instructionQueue([&](Value* lhs, Value* rhs) -> bool {
        ...
        for (auto inst = blockInstructionsRaw.begin(); *inst != blockInstructionsRaw.back(); inst++){
            if (isNoSideEffectExpr(**inst)){                
                ...
                while (!blockInstructionsCurrent.empty()){
                    for (auto instCheck = blockInstructionsCurrent.begin(); instCheck != blockInstructionsCurrent.end(); instCheck++){   
                        bool effected = false;   
