    ...
    bool runOnBlock(Block& block) const {
        return reduceBlock(block, [](Instruction* inst, std::unordered_map<Value*, Value*>& replace) -> Value* {
            ...
            if(srem(any(v1), cint_(1))(matchCtx) || urem(any(v1), cuint_(1))(matchCtx))
                return make<ConstantInteger>(inst->getType(), 0);
            // a * c + b * c -> (a + b) * c
            if(add(mul(any(v1), any(v2)), mul(any(v3), any(v4)))(matchCtx) && v2 == v4) {
            ...
            if(add(any(v1), any(v2))(matchCtx) && v1 == v2)
                return make<BinaryInst>(InstructionID::Mul, inst->getType(), make<ConstantInteger>(v1->getType(), 2), v1);
            // b * a + a -> (b+1) * a
            if(add(mul(any(v1), any(v2)), any(v3))(matchCtx)) {
                ...
                if(a && b)
                    return make<BinaryInst>(
                        InstructionID::Mul, inst->getType(),
                        make<BinaryInst>(InstructionID::Add, inst->getType(), b, make<ConstantInteger>(v1->getType(), 1)), a);
            }
            // b * a + c * a -> (b + c) * a
            if(add(mul(any(v1), any(v2)), mul(any(v3), any(v4)))(matchCtx)) {
                ...
                if(a && b && c)
                    return make<BinaryInst>(InstructionID::Mul, inst->getType(),
                                            make<BinaryInst>(InstructionID::Add, inst->getType(), b, c), a);
            }
            return nullptr;
        });
    }
